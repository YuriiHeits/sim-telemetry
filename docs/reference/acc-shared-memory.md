...
## Розбіжність між джерелами — вирішена

`surfaceGrip`: офіційна дока Kunos описує його як «Ideal line friction
coefficient», PyAccSharedMemory коментує як «Return always 0».

**Права PyAccSharedMemory.** Живий замір на трасі (Red Bull Ring, M4 GT3,
sm 1.9 / build 1.7) дав рівно `0.000` на всіх семплах. Колонки `grip` немає і не
буде, поки хтось не покаже ненульове значення на іншій версії гри. Перевірити
можна одною командою: `python probe_sim.py` на треку.

Мораль на майбутнє: офіційна дока описує *задум*, а не те, що гра справді
заповнює. Там, де дока і робоча реалізація розходяться, вирішує замір.
